...
class TeamGitHubDataSource(TeamDataSource):
    ...
    def get(self) -> Sequence[TeamModel]:
        ...
        expected_keys = {f.name for f in TeamModel.__dataclass_fields__.values()}

        required_data = []
        for item in data:
            item_filtered = filter_dict(item, list(expected_keys))
            item_typed = coerce_types(TeamModel, item_filtered)  # csv has all values as string
            required_data.append(TeamModel(**item_typed))
...
class TeamGitHub(BaseTeamRepository[Team]):
    def __init__(self, season: str = "2024-25") -> None:
        super().__init__(Team, TeamGitHubDataSource(season=season))
