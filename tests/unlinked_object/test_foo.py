...
from fplpy.objects._element.element import ElementTemplate
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Fixture, wrap_argument(fixture_model()), "14 v 9"),
...
@pytest.mark.parametrize(
    ...
    [
        (Team, wrap_argument(team_model()), "Team(code(ID)=3, name='Arsenal')"),
        (Label, wrap_argument(label_model()), "Label(label='Goals Scored', name='goals_scored')"),
        (Position, wrap_argument(position_model()), "Position(id(ID)=1, singular_name='Goalkeeper', singular_name_short='GKP')"),
        (Player, wrap_argument(player_model()), "Player(code(ID)=438098, web_name='Fábio Vieira', team=1, position=3)"),
        (Fixture, wrap_argument(fixture_model()), "Fixture(code(ID)=2444470, team_h=14, team_a=9, event=1)"),
        (Event, wrap_argument(event_model()), "Event(id(ID)=1, name='Gameweek 1', deadline_time='2024-08-16T17:30:00Z')")
    ]
)
